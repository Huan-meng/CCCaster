...
void NetplayManager::startInputPrefetch()
{
    if (_inputPrefetchRunning.load())
    {
        log("Input prefetch thread already running");
        return;
    }

    log("Starting input prefetch thread");
    _inputPrefetchShouldStop.store(false);
    _inputPrefetchRunning.store(true);
    _inputPrefetchThread = std::thread(&NetplayManager::inputPrefetchLoop, this);
}

void NetplayManager::stopInputPrefetch()
{
    if (!_inputPrefetchRunning.load())
    {
        return;
    ...
    _inputPrefetchShouldStop.store(true);

    if (_inputPrefetchThread.joinable())
    {
        _inputPrefetchThread.join();
    }

    _inputPrefetchRunning.store(false);
}

void NetplayManager::inputPrefetchLoop()
{
    log("Input prefetch thread started");

    while (!_inputPrefetchShouldStop.load())
    {
        // 使用 poll() 代替 checkEvents()，因为 checkEvents() 是私有方法
        // poll() 会调用 checkEvents() 并处理所有事件
        if (EventManager::get().isRunning())
        {
            // 非阻塞检查（timeout = 0 立即返回）
            EventManager::get().poll(0);
        }
    ...
    log("Input prefetch thread stopped");
}
